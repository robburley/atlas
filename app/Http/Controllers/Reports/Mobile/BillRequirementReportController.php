...
class BillRequirementReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = collect([
            [
                'name' => 'Total',
                'bill' => $dayBill = MobileOpportunity::whereHas('mobileBills', function ($q) use ($start, $end) {
                    $q->whereBetween('created_at', [$start, $end]);
                })->count(),
                'noBill' => $dayNoBill = MobileOpportunity::whereBetween('no_bill_date', [$start, $end])->where('no_bill', 1)->count(),
        ...
        foreach ($users as $user) {
            $data = $data->merge([
                [
                    'name' => $user->name,
                    'bill' => $dayBill = $user->createdMobileOpportunities()->whereHas('mobileBills',
                        function ($q) use ($start, $end) {
                            $q->whereBetween('created_at', [$start, $end]);
                        })->count(),
                    'noBill' => $dayNoBill = $user->createdMobileOpportunities()->where('no_bill', 1)->whereBetween('no_bill_date', [$start, $end])->count(),
                    'total' => $todayTotal = $dayBill + $dayNoBill,
                    'billPercent' => $todayTotal > 0 ? number_format(($dayBill / $todayTotal) * 100, 2) : '--',
                    'noBillPercent' => $todayTotal > 0 ? number_format(($dayNoBill / $todayTotal) * 100, 2) : '--',
                ]
