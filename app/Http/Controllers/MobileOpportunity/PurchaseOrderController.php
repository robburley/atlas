...
class PurchaseOrderController extends Controller
{
    ...
    public function destroy(Customer $customer, MobileOpportunity $opportunity)
    {
        if ($opportunity->adobeSignDocumentPurchaseOrder && $opportunity->adobeSignDocumentPurchaseOrder->status != 'SIGNED') {
            $opportunity->adobeSignDocumentPurchaseOrder->delete();

            alert()->error('Purchase order deleted, please resend.');
        } else {
            alert()->error('Purchase order has already been signed.');
        }

        return redirect()->back();
    }

    public function store(Customer $customer, MobileOpportunity $opportunity, Request $request)
    {
        ...
        if (!auth()->user()->hasPermission('ignore_deal_calculator_authorisation')) {
            $overview = $opportunity->selectedDealCalculator->first()->overview;

            if (!$overview || ($overview->profitMargin < 30 && $overview->discountMargin > 65)) {
                alert()->error('This deal calc is not authorised, please resolve before sending the Purchase Order');
        ...
        try {
            ...
            alert()->error(
                'Please try again, if this persists, contact support quoting: ' . $e->getMessage(),
                'An Error occurred'
            )->confirmButton();
        }
    ...
    public function createPdf(Customer $customer, MobileOpportunity $opportunity, Request $request)
    {
        ...
        SnappyPdf::loadView('mobile.opportunities.pdf.purchase-order', [
