...
@section('fulfilment-content')
    ...
    {!! Form::open(['action' => ['MobileOpportunity\Fulfilment\ConnectionDeferredController@store', $customer, $opportunity], 'method' => 'post', 'files' => true]) !!}
    <div class="row">
        <div class="col-sm-12">
            <table class="table">
                <tr>
                ...
                @foreach($opportunity->allocations as $allocation)
                    @if($allocation->connection_deferred)
                        <tr>
                            <td class="v-mid">
                                {{ $allocation->type }}
                            </td>
    ...
    <div class="row hidden" id="error-div">
        <div class="col-sm-2 col-sm-offset-10">
            ...
            <label>
                <input type="checkbox" name="errors[PAC Code doesn't match CTN (s)]" value="1"> PAC Code doesn't match CTN (s)
            </label>
            ...
            <label>
                <input type="checkbox" name="errors[BCAD Doesn't match deal structure]" value="1"> BCAD Doesn't match deal structure
            </label>

            <label>
                <input type="checkbox" name="errors[Credit Check decision has expired]" value="1"> Credit Check decision has expired
            </label>
    ...
    <div class="row hidden" id="passed-div">
        <div class="col-sm-3 col-sm-offset-9">
            ...
            {!! Form::text('date_connected', \Carbon\Carbon::now()->format('d/m/Y'), ['class' => 'form-control datepicker', 'readonly']) !!}

            {!! Form::label('contract_end_date', 'Contract End Date', ['class' => 'control-label']) !!}
            {!! Form::text('contract_end_date', \Carbon\Carbon::now()->addYears(3)->format('d/m/Y'), ['class' => 'form-control datepicker', 'readonly']) !!}
