...
class CloserReportController extends Controller
{
    ...
    public function getData($start, $end)
    {
        $users = User::active()->whereHas('permissions', function ($query) {
            $query->where('permissions.slug', 'assignable_mobile');
        })
            ->get();
        ...
        $data = $users->map(function ($user) use ($start, $end, &$count) {
            ...
            $issuedTotal = $user->mobileAssigned()->dateRange($start, $end)->get()->unique('id')->count();
            $total = $user->activeMobileAssigned()->dateRange($start, $end)->count();
            $awaitingQual = $user->activeMobileAssigned()
                ->status([
                ...
                ->dateRange($start, $end)
                ->count();
            $callback = $user->activeMobileAssigned()->status(['Awaiting Callback'])->dateRange($start, $end)->count();
            $qualified = $user->activeMobileAssigned()->qualified()->dateRange($start, $end)->count();
            $notQualified = $user->activeMobileAssigned()->notQualified()->dateRange($start, $end)->count();
            $dealt = $user->activeMobileAssigned()
                ->status([
                ...
                ->dateRange($start, $end)
                ->count();
            $passed = $user->activeMobileAssigned()->status(['Passed Credit Check'])->dateRange($start, $end)->count();
            $blown = $user->activeMobileAssigned()->blown()->dateRange($start, $end)->count();
